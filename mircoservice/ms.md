...
假设有5台redis，一个请求来加锁，前三个都成功了，但是很不巧，第三个挂了，挂了之后马上运维就起了一台新redis，这台新redis是台空的，这个时候又来了一个请求,它也一通加锁，由于新启了一个redis所以，它也在超过半数的redis中加锁成功了

这就导致又回到了超卖的问题

解决这个问题：在部署的时候写一个部署手册，里面提到一点，如果这5个redis其挂点一台，不要立马重启，过一段时间再重启，这个时间要么是key都过期了，要么请求都把锁释放掉了

**redis的终极问题**

当一个请求来获取到了锁，并且派了一个看门狗，但这个时候非常不巧的是，JVM恰好stw了
这时候狗也白搭，而由于看门狗也停住了导致key过期，而恢复之后，又继续了业务
而同时另一个请求也拿到了锁，这就又回到了超卖的问题

使用zk来解决
利用zk做锁需要了解zk的两个特性：顺序节点，临时节点：
临时节点：如果我的程序和zk进行连接，连接还在节点就在，连接断了，节点就断了
顺序节点：第一次加锁的节点是1号节点，再来一个程序加锁节点就变成了2
再来加锁节点就变成了3，节点是顺序递增的

通过zk的顺序节点，并且是临时节点，当一个请求加锁成功，建立了一个节点，
比如说节点是1号，如果这个时候发生了stw，请求和节点之间断开连接了，节点就会消失，这个时候2号请求来加锁成功了，会产生一个2号节点，之前1号请求加锁成功会将信息存到数据库中

### 分布式事务

事务：事务ACID：原子性，一致性，隔离性，持久性
事务就是要么全部成功，要么全部事务

分布式事务：分布式事务，指事物的参与者，支持事物的服务器，资源服务器以及事物管理器分别位于不同的分布式系统的不同节点上，且属于不同的应用，分布式事务需要保证这些操作要么全部成功，要么全部失败。本质上分布式事务就是为了保证不同数据库的数据一致性

#### 分布式事务解决方案

Seate是一套开源的分布式事务解决方案，致力于提供高性能和简单易用的分布式事务服务
